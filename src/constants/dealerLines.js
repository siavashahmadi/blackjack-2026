...
export const DEALER_LINES = {
  ...
  tableLevelUp: [
    (ctx) => `Welcome to ${ctx.tableName}. Try not to embarrass yourself.`,
    (ctx) => `${ctx.tableName}. The stakes just got real.`,
    (ctx) => `Moving up to ${ctx.tableName}. The chips are bigger and so are the losses.`,
    (ctx) => `${ctx.tableName} awaits. Don't get comfortable.`,
    (ctx) => `You've earned your seat at ${ctx.tableName}. For now.`,
    (ctx) => `${ctx.tableName}. The air is different up here. Smells like regret.`,
    (ctx) => `Welcome to ${ctx.tableName}. The minimum bet alone would make your mother cry.`,
    (ctx) => `Ah, ${ctx.tableName}. Where dreams come to die in style.`,
  ],
  tableLevelDown: [
    ...
    (ctx) => `${ctx.tableName}. Where you belong. I mean, welcome back.`,
  ],
}
